...
pmap_db_dump_kvm(void)
{
	...
	struct pmap_lev1 *pml1;
	struct pmap_lev2 *pml2;
	unsigned pml0i, pml1i, pml2i, ptei;
	struct pmap *pm;
	pt_entry_t *pte;
	vaddr_t addr;

	pm = kernel_vm.vm_pmap;
	ASSERT(pm != NULL, "need a pmap to dump!");

	kcprintf("Beginning kernel virtual address page mapping dump...\n");
	for (pml0i = 0; pml0i < NL0PMAP; pml0i++) {
		pml0 = pm->pm_level0[pml0i];
		if (pml0 == NULL)
			continue;
		kcprintf("> L0 %u (%p)\n", pml0i, (void *)pml0);
		for (pml1i = 0; pml1i < NL1PL0; pml1i++) {
			pml1 = pml0->pml0_level1[pml1i];
			if (pml1 == NULL)
				continue;
			kcprintf(">> L1 %u (%p)\n", pml1i, (void *)pml1);
			for (pml2i = 0; pml2i < NL2PL1; pml2i++) {
				pml2 = pml1->pml1_level2[pml2i];
				if (pml2 == NULL)
					continue;
				kcprintf(">>> L2 %u (%p)\n", pml2i, (void *)pml2);
				for (ptei = 0; ptei < NPTEL2; ptei++) {
					pte = &pml2->pml2_entries[ptei];
					if (!pte_test(pte, PG_V))
						continue;
					addr = pm->pm_base;
					addr += pml0i * PMAPL0DIV;
					addr += pml1i * L1L0DIV;
					addr += pml2i * L2L1DIV;
					addr += ptei * PTEL2DIV;
					kcprintf(">>>> %p -> %lx (%x)\n",
